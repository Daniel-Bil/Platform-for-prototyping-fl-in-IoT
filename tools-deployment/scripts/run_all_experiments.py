...
def validate_run(rd:Path,clients:int,rounds:int):
    ...
    if not (s.get("final_metrics") or {}).get("test_samples"): raise RuntimeError("final distributed evaluation is missing")
    ...
    for n in counts:
      for rep in range(1,a.repetitions+1):
        ...
        for alg in algs:
            ...
            try:
                ev=[f"fl_client_count={n}",f"fl_requested_clients={n}",f"fl_algorithm={alg}",f"fl_rounds={a.rounds}",f"fl_local_epochs={a.local_epochs}",f"fl_batch_size={a.batch_size}",f"fl_seed={seed}",f"fl_fedprox_mu={a.fedprox_mu}",f"fl_fedpaq_bits={a.fedpaq_bits}",f"fl_edge_count={min(a.edge_count,n)}",f"fl_initial_join_window={a.initial_join_window}",f"fl_join_window={a.join_window}",f"fl_run_id={run_id}",f"fl_campaign_id={campaign_id}",f"fl_repetition={rep}"]
