...
async def handle_client(reader, writer, shared_state):
    ...
    for method in shared_state['methods']:
        for iteration in range(shared_state['iterations']):
            ...
            if method == "fedavg" or method =="fedprox":
                ...
                if iteration == 0:
                    # In the first iteration, send the initially trained model weights
                    ...
                    # For subsequent iterations, send the averaged global weights
                    data_to_send = json.dumps({
                        "header": "2",
                        "name": shared_state["current_model_name"],
                        "method": method,
...
async def main():
    ...
    for architecture in loaded_architectures:
        ...
        print(f"Training model on the server for architecture {architecture['name']}")
